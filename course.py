...
def patch2(win,colour3,size,x,y):
    win= Window("",500,500)
    colour3 = "red"
    size = 500
    test = Point(x,y)
    start_x = test.x
    start_y = test.y
    for Y in range(100,start_y+size-100,10):
        for X in range(100,start_x+size-100,10):
            tl = Point(Y,X)
            mod = Y % 100
            if mod % 2 == 0:
                br = Point(Y+5,X+5)
                ...
                br = Point(Y+10,X+10)
        rectangle(win,tl,br,"black",colour3)
