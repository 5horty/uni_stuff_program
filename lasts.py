...
def check_keys(win,grid,colour,colour2,colour3):
    border_grid = {}
    click = win.get_mouse()
    ...
    while True:
        ...
        match key:
            ...
            case "7":
                if (patch_x,patch_y)not in grid:
                    patch = draw_final_patch(win,Point(patch_x,patch_y),Point(patch_x+100,patch_y+100),colour,colour)
                    grid[(patch_x,patch_y)] = patch
            case "8":
                if (patch_x,patch_y)not in grid:
                    patch = rectangle(win,Point(patch_x,patch_y),Point(patch_x+100,patch_y+100),colour2,colour2)
                    grid[(patch_x,patch_y)] = patch
            case "9":
                if (patch_x,patch_y)not in grid:
                    patch = rectangle(win,Point(patch_x,patch_y),Point(patch_x+100,patch_y+100),colour3,colour3)
                    grid[(patch_x,patch_y)] = patch
            case "Up":
...
def loops(win,size,colour,colour2,colour3):
    ...
    check_keys(win,grid,colour,colour2,colour3)
